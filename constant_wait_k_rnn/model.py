...
from utils import batch_iter, LabelSmoothedCrossEntropy, StackedLSTM
import time
...
class LSTMSeq2seq(nn.Module):
    ...
    def __init__(self, embedding_size, hidden_size, vocab, bidirectional=False, dropout_rate=0.3, label_smooth=1.0,
                 num_layers=2, wait_k = 3):
        ...
        # self.encoder_lstm = nn.LSTM(embedding_size, hidden_size, rdrop=dropout_rate, bidirectional=False)
        self.encoder_lstm = StackedLSTM(num_layers, embedding_size, hidden_size, dropout=dropout_rate)
    ...
    def encode(self, src_tokens, src_lens):
        ...
        src_vectors = self.src_embedding(src_tokens).permute(1,0,2)  # (max_seq_len, batch_size, embedding_size)
        batch_size = src_vectors.size(1)
        max_seq_len = src_vectors.size(0)
        ...
        if src_tokens.is_cuda:
            hidden = [[torch.zeros(batch_size, self.hidden_size).cuda(), torch.zeros(batch_size, self.hidden_size).cuda()], \
                [torch.zeros(batch_size, self.hidden_size).cuda(), torch.zeros(batch_size, self.hidden_size).cuda()]]

        outputs = []
        length = 0
        for src_vector in src_vectors.split(1):
            src_vector = src_vector.squeeze(0)

            output, hidden = self.encoder_lstm(src_vector, hidden)
            outputs += [output]

            if length == self.wait_k - 1 or (max_seq_len < self.wait_k and length == max_seq_len - 1):
                final_cell_states = hidden[1]
                c = self.enc_final_to_dec_init(final_cell_states.contiguous().view(batch_size, -1))
                h = torch.tanh(c)
            length += 1

        src_states = torch.stack(outputs).permute(1, 0, 2) # batch_size x max_seq_len x hidden_size
        return src_states, (h, c)
    ...
    def decode(self, src_states, final_states, src_lens, trg_tokens, trg_lens, teacher_forcing=0.5):
        ...
        for t in range(trg_tokens.size(-1) - 2):
            ...
            context_vector = self.dropout(
                LSTMSeq2seq.compute_attention(h, src_states[:, :self.wait_k + t + 1, :], src_lens, attn_func=self.attn_func))
            curr_attn_vector = self.dropout(
                self.decoder_hidden_layer(torch.cat((h, context_vector), dim=-1)))  # the thing to feed in input feeding
            curr_logits = self.decoder_output_layer(curr_attn_vector)  # (batch_size, vocab_size)
    ...
    def greedy_search(self, src_sent, src_lens, beam_size=5, max_decoding_time_step=70, cuda=True):
        ...
        while decoding_step <= max_decoding_time_step and prd_token.item() != END_TOKEN_IDX:
            ...
            h, c = self.decoder_lstm_cell(vector, (h, c))
            context_vector = self.dropout(
                LSTMSeq2seq.compute_attention(h, src_states[:, :self.wait_k + decoding_step, :], src_lens, attn_func=self.attn_func))
            curr_attn_vector = self.dropout(
                self.decoder_hidden_layer(torch.cat((h, context_vector), dim=-1)))  # the thing to feed in input feeding
            curr_logits = self.decoder_output_layer(curr_attn_vector)  # (batch_size, vocab_size)
            curr_ll = F.log_softmax(curr_logits, dim=-1)  # transform logits into log-likelihoods
