      ...
      for i in range(len(chunks)):
         for j in range(i+1, len(chunks)):
            if not chunk_alignments[j] or not chunk_alignments[i] :
               continue
            if mask[j] == False or mask[i] == False:
               continue
...
class EnSentence(Sentence):
   ...
   def get_vw_feat_stat(self, word_rank, no_punct=True):
      ...
      # prefix
      #prefix = ['anti', 'auto', 're', 'non', 'post', 'pre', 'un']
      #suffix = ['ish', 'able', 'ness', 'less', 'ity', 'tion', 'ment']
      #npre = 0
      #nsuf = 0
      #for pre in prefix:
      #   for tok in toks:
      ...
      #         npre += 1
      #   feats.append(('prefix_%s_num' % pre, npre / N))

      #for suf in suffix:
      #   for tok in toks:
      #      if tok.lower().endswith(suf):
      #         nsuf += 1
      #   feats.append(('suffix_%s_num' % suf, nsuf / N))
...
class SentencePair:
   ...
   def compute_bilingual_feats(self):
      ...
      #mask = [True if sent.words[chunk[0]].ctag == u'動詞' else False for chunk in chunks]
      #invs, npairs = sent.get_inversion(chunks, mask)
      #feat['num_invs_verb'] = 0 if npairs == 0 else len(invs) / float(npairs)

      #mask = [True if sent.words[chunk[0]].ctag == u'名詞' else False for chunk in chunks]
      #invs, npairs = sent.get_inversion(chunks, mask)
      #feat['num_invs_noun'] = 0 if npairs == 0 else len(invs) / float(npairs)
